...
import {
  ...
  validateToolConnectionCredentials,
  type ToolConnectionCredentials,
...
router.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      console.error(
        "Failed to get tool connections:",
        error,
      );
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const validatedIntegration =
        await validateToolConnectionCredentials(
          integration.id,
          credentials as ToolConnectionCredentials,
        );
      ...
      return res.status(201).json({
        connection: {
          ...
          providerId:
            validatedIntegration.provider_id,
          connectionName:
